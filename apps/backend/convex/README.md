...
- `http.ts`: HTTP router (`/clerk/webhook`, `/whatsapp-cloud/webhook`, `/telegram/webhook`).
...
- `delivery.ts`: Outbound queue claim/complete/fail flow (web/WhatsApp/Telegram egress paths).
- `telegram/`: Telegram inbound webhook + queueing mutations.
...
- `TELEGRAM_WEBHOOK_SECRET` (required for Telegram inbound webhook verification)
